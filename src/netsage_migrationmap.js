...
export class NetsageMigrationMap extends MetricsPanelCtrl {
    ...
    */
		...
		if (document.getElementById(ctrl.migrationmap_holder_id)) {
			document.getElementById(ctrl.containerDivId).innerHTML =
				'<div id="map"><div id = "legend" >LEGEND </br> </div></div>';
			...
			filteredData.forEach((element) => {
				var lineToAdd =
					'<span class = "legendItem" style= "color: ' +
					element.color +
					';">' +
					element.name +
					'</span></br>';
			...
			if (ctrl.panel.showControls) {
				...
				controlsDiv.innerHTML +=
					'<label class="switch"><input type="checkbox" id="playPause"><span class="slider round"></span></label>';
			}
			...
			filteredMarkers.forEach((element) => {
				...
				var existsAsSource = data.filter(
					({ from }) => from[0] === element.latlong[1] && from[1] === element.latlong[0]
				);
				...
				if (existsAsSource.length > 0) {
					...
					for (var i = 0; i < existsAsSource.length; i++) {
						if (existsAsSource[i]) {
							...
							outGoingDiv +=
								"<span style = 'color: " +
								existsAsSource[i].color +
								"'> Discipline : " +
								existsAsSource[i].name +
								'  </br> To : ' +
								existsAsSource[i].destDisplayName +
								'</br> Resource : ' +
								existsAsSource[i].destResourceDisplayName +
								' </br> Volume : ' +
								formattedValue +
								'</span></br></br>';
						}
				...
				var existsAsDest = data.filter(
					({ to }) => to[0] === element.latlong[1] && to[1] === element.latlong[0]
				);
					...
					for (var i = 0; i < existsAsDest.length; i++) {
						if (existsAsDest[i]) {
							...
							if (existsAsDest[i].labels[0] === existsAsDest[i].labels[1]) {
								...
								incomingDiv +=
									"<span style = 'color: " +
									existsAsDest[i].color +
									"'> Discipline : " +
									existsAsDest[i].name +
									'  </br> From : ' +
									existsAsDest[i].labels[0] +
									'</br> Resource : ' +
									existsAsDest[i].srcResourceName +
									'</br>Volume : ' +
									formattedValue +
									'</span></br></br>';
							}
